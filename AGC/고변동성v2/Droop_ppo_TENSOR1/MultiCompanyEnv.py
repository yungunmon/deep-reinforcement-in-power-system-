...
sys_path_PSSE=r'C:\Program Files\PTI\PSSE35\35.2\PSSPY38'  #or where else you find the psspy.pyc
...
0.5254237288135593 ,0.5254237288135593 ,0.5084745762711864,0.38983050847457623 ] 
    ...
    def noact(self, PV_,WT_):
        ...
        PVcor = self.PV[(self.time+1)%self.MaxTime]*(1+((np.random.rand()-0.5)*2)*0.05)
        WTcor = self.Wind[(self.time+1)%self.MaxTime]*(1+((np.random.rand()-0.5)*2)*0.1)

...
net5 = CommNetActor(5,save_path)
net6 = CommNetActor(6,save_path)
net7 = CommNetActor(7,save_path)
'''
...
if train_mode:  
    ...
    for ep in range(EP_MAX):
        ...
        for t in range(EP_LEN):
            ...
            prob5 = net5.pi(torch.from_numpy(state).float())
            prob6 = net6.pi(torch.from_numpy(state).float())
            prob7 = net7.pi(torch.from_numpy(state).float())
            ...
            m1 = torch.nn.functional.softmax(prob1[0], dim=-1).detach().numpy()
            m2 = torch.nn.functional.softmax(prob2[0], dim=-1).detach().numpy()
            m3 = torch.nn.functional.softmax(prob3[0], dim=-1).detach().numpy()
            m4 = torch.nn.functional.softmax(prob4[0], dim=-1).detach().numpy()
            m5 = torch.nn.functional.softmax(prob5[0], dim=-1).detach().numpy()
            m6 = torch.nn.functional.softmax(prob6[0], dim=-1).detach().numpy()
            m7 = torch.nn.functional.softmax(prob7[0], dim=-1).detach().numpy()
            '''
            ...
            net1.put_data((state, a1, rewards[0], state_next, prob_a1, terminals[0]))
            net2.put_data((state, a2, rewards[1], state_next, prob_a2, terminals[1]))
            net3.put_data((state, a3, rewards[2], state_next, prob_a3, terminals[2]))
            net4.put_data((state, a4, rewards[3], state_next, prob_a4, terminals[3]))
            net5.put_data((state, a5, rewards[4], state_next, prob_a5, terminals[4]))
            net6.put_data((state, a6, rewards[5], state_next, prob_a6, terminals[5]))
            net7.put_data((state, a7, rewards[6], state_next, prob_a7, terminals[6]))
            # ===================================
   ...
   for t in range(EP_LEN):
      ...
      acts5 = a5 * 0.1   - 1           #sop2(27-54) -1 ~ 1
      acts6 = a6 * 0.03  - 0.3         #shunt69 -0.3 ~ 0.3
      acts7 = a7 * 0.03  - 0.3            #shunt15 -1.1 ~ 0.1

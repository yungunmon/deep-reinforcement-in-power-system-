...
def mu_variable(shape):
    return tf.Variable(tf.random.uniform(shape, minval = -tf.sqrt(3/shape[0]), maxval = tf.sqrt(3/shape[0])))
...
def noisy_dense(input_, input_shape, mu_w, sig_w, mu_b, sig_b, is_train_process):
    eps_w = tf.cond(is_train_process, lambda: tf.random.normal(input_shape), lambda: tf.zeros(input_shape))
    eps_b = tf.cond(is_train_process, lambda: tf.random.normal([input_shape[1]]), lambda: tf.zeros([input_shape[1]]))
...
if __name__ == "__main__":
    state = tf.compat.v1.placeholder(tf.float32, [None, 3, 4], 'state')
    CommNet(state, 4, tf.compat.v1.placeholder(tf.bool))
...
class IAC(object):
    def __init__(self, save_path):
        ...
        self.actions_old = tf.placeholder(tf.float32, [None, action_size, AGENT_NUM], 'actions_old')
        ...
        self.ratio = tf.reduce_sum(self.actor_out*self.actions,1,keep_dims=True) / tf.reduce_sum(self.actions_old* self.actions,1,keep_dims=True) 
        ...
        self.surrogate1 = self.ratio * self.advantages
        self.surrogate2 = tf.clip_by_value(self.ratio,0.9,1.1) * self.advantages

        self.a_loss = - tf.reduce_mean(tf.minimum(self.surrogate1, self.surrogate2))
    ...
    def build_actor(self, name, trainable):
        with tf.variable_scope(name):
            ...
            for temp_out in inter_out_list:
                action_prob_list.append(tf.nn.softmax(tf.nn.leaky_relu(temp_out)))
    ...
    def train_op(self, s, s_next, a, r, p , t, c_lr=0.0001, a_lr=0.0001):
        ...
        #print('b_adv=',b_adv)
        b_p = np.vstack(b_prob)
        
        #aout = self.sess.run(self.actor_out, {self.state: s,  self.is_train: True})

        #print('aout* b_a=', aout* b_a)
        #print('actions_old=', b_p)
        
        #print(self.sess.run(self.surrogate1,{self.state: b_s, self.actions: b_a, self.actions_old : b_p  , self.advantages: b_adv, self.a_lr: a_lr,
        #                           self.is_train: True}))

        _, a_loss = self.sess.run([self.a_train, self.a_loss],
                                  {self.state: b_s, self.actions: b_a, self.actions_old : b_p  , self.advantages: b_adv, self.a_lr: a_lr,
                                   self.is_train: True})

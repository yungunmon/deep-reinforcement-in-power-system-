...
class PPO(nn.Module):
    ...
    def make_batch(self):
        ...
        s,a,r,s_prime,done_mask, prob_a = torch.tensor(s_lst, dtype=torch.float).to(device), torch.tensor(a_lst, dtype=torch.int64).to(device), \
                                          torch.tensor(r_lst, dtype=torch.float).to(device), torch.tensor(s_prime_lst, dtype=torch.float).to(device), \
                                          torch.tensor(done_lst, dtype=torch.float).to(device), torch.tensor(prob_a_lst, dtype=torch.float).to(device)
        self.data = []

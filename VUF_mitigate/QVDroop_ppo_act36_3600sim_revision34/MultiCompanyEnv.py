...
path = 'C:/Users/yungun/Desktop/labsil/Distributed_unbalance_mitigate/QVDroop_ppo_act36_3600sim_revision34/MajorRevision/'
...
class env():
    ...
    def reset(self,ra):
        ...
        state806 = np.hstack([a, self.Load[self.time] ]).flatten()
        state810 = np.hstack([a, self.Load[self.time] ]).flatten()
        state812 = np.hstack([a, self.Load[self.time] ]).flatten()
        state818 = np.hstack([a, self.Load[self.time] ]).flatten()
        state826 = np.hstack([a, self.Load[self.time] ]).flatten()
        state828 = np.hstack([a, self.Load[self.time] ]).flatten()
        state856 = np.hstack([a, self.Load[self.time] ]).flatten()
        state834 = np.hstack([a, self.Load[self.time] ]).flatten()
        state890 = np.hstack([a, self.Load[self.time] ]).flatten()
        state840 = np.hstack([a, self.Load[self.time] ]).flatten()
        state836 = np.hstack([a, self.Load[self.time] ]).flatten()
        state860 = np.hstack([a, self.Load[self.time] ]).flatten()

        states = [state806, state810, state812, state818, state826, state828, state856, state834, state890, state840, state836, state860]
    ...
    def step(self, acts):        
        ...
        next_state = n_state
        total_reward = [self.rwd[0], self.rwd[1], self.rwd[2], self.rwd[3], self.rwd[4], self.rwd[5], self.rwd[6], self.rwd[7], self.rwd[8], self.rwd[9], self.rwd[10], self.rwd[11]]
        terminals = [False,False,False,False,False,False,False,False,False,False,False,False]
        info = ifo
        ...
        if self.time == self.MaxTime:
            ...
            terminals = [True,True,True,True,True,True,True,True,True,True,True,True]
    ...
    def _step(self,acts):
        ...
        for j in range(6):
            ...
            reward += VUF - (abs(noVUFn[0]) + abs(noVUFz[0]))
        ...
        state806 = np.hstack([self.PV[(self.time*6+6)%self.totaltime], self.Load[(self.time*6+6)%self.totaltime] ]).flatten()
        state810 = np.hstack([self.PV[(self.time*6+6)%self.totaltime], self.Load[(self.time*6+6)%self.totaltime] ]).flatten()
        state812 = np.hstack([self.PV[(self.time*6+6)%self.totaltime], self.Load[(self.time*6+6)%self.totaltime] ]).flatten()
        state818 = np.hstack([self.PV[(self.time*6+6)%self.totaltime], self.Load[(self.time*6+6)%self.totaltime] ]).flatten()
        state826 = np.hstack([self.PV[(self.time*6+6)%self.totaltime], self.Load[(self.time*6+6)%self.totaltime] ]).flatten()
        state828 = np.hstack([self.PV[(self.time*6+6)%self.totaltime], self.Load[(self.time*6+6)%self.totaltime] ]).flatten()
        state856 = np.hstack([self.PV[(self.time*6+6)%self.totaltime], self.Load[(self.time*6+6)%self.totaltime] ]).flatten()
        state834 = np.hstack([self.PV[(self.time*6+6)%self.totaltime], self.Load[(self.time*6+6)%self.totaltime] ]).flatten()
        state890 = np.hstack([self.PV[(self.time*6+6)%self.totaltime], self.Load[(self.time*6+6)%self.totaltime] ]).flatten()
        state840 = np.hstack([self.PV[(self.time*6+6)%self.totaltime], self.Load[(self.time*6+6)%self.totaltime] ]).flatten()
        state836 = np.hstack([self.PV[(self.time*6+6)%self.totaltime], self.Load[(self.time*6+6)%self.totaltime] ]).flatten()
        state860 = np.hstack([self.PV[(self.time*6+6)%self.totaltime], self.Load[(self.time*6+6)%self.totaltime] ]).flatten()

        next_states = [state806, state810, state812, state818, state826, state828, state856, state834, state890, state840, state836, state860]
        ...
        infoA = [act1,act2,act3,act4,act5,act6,act7,act8,act9,act10,act11,act12]

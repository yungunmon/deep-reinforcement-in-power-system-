...
save_path = "C:/Users/yungun/Desktop/labsil/Distributed_unbalance_mitigate/QVDroop_ppo_act36_3600sim_revision34/MajorRevision/"
train_mode = True
...
net10 = PPO(10,save_path)
net11 = PPO(11,save_path)
net12 = PPO(12,save_path)
...
net10.load_state_dict(torch.load(os.path.join(save_path,'net10_params.pkl')))
net11.load_state_dict(torch.load(os.path.join(save_path,'net11_params.pkl')))
net12.load_state_dict(torch.load(os.path.join(save_path,'net12_params.pkl')))
...
if train_mode:
  for ep in range(EP_MAX):
    ...
    for t in range(EP_LEN):
      ...
      normalizer.observe(state[9])
      normalizer.observe(state[10])
      normalizer.observe(state[11])
      ...
      prob10 = net10.pi((torch.from_numpy(state10).float()).to(device))
      prob11 = net11.pi((torch.from_numpy(state11).float()).to(device))
      prob12 = net12.pi((torch.from_numpy(state12).float()).to(device))
      ...
      m10 = Categorical(prob10)
      m11 = Categorical(prob11)
      m12 = Categorical(prob12)
      ...
      prob_a10 = prob10[a10].item()
      prob_a11 = prob11[a11].item()
      prob_a12 = prob11[a12].item()
      action = [a1, a2, a3, a4, a5, a6, a7, a8, a9, a10, a11, a12] #ENV에서 플러스 2하기
      ...
      state_next10 = state_next[9]
      state_next11 = state_next[10]
      state_next12 = state_next[11]
      ...
      proba[a8][7] = prob_a8
      proba[a9][8] = prob_a9
      proba[a10][9] = prob_a10
      proba[a11][10] = prob_a11
      proba[a12][11] = prob_a12
      ...
      net10.put_data((state10, a10, rewards[9], state_next10, prob_a10, terminals[9]))
      net11.put_data((state11, a11, rewards[10], state_next11, prob_a11, terminals[10]))
      net12.put_data((state12, a12, rewards[11], state_next12, prob_a12, terminals[11]))
      ...
      # print(rewards)
      # Train network ================================================================================================
      if (t + 1) % 500 == 0 or t == EP_LEN - 1 and train_mode:
        net1.train_net()
        ...
        net10.train_net()
        torch.save(net10.state_dict(),os.path.join(save_path ,'net10_params.pkl'))
        net11.train_net()
        torch.save(net11.state_dict(),os.path.join(save_path ,'net11_params.pkl'))
        net12.train_net()
        torch.save(net12.state_dict(),os.path.join(save_path ,'net12_params.pkl'))

...
def add_comment(request):
    if request.method == "POST":
        ...
        news.save()
        return JsonResponse({'comment' : comment,'pk':pk})
...
def crawl(request):
	...
	def news_crowler(Keyword_list, Select) : 
		...
		for i in range(len(Keyword_list)): #리스트의 키워드를 반복하기 위한 For 반복문#
			...
			link_partition_3 = ', -스포츠 , -손흥민, -농구, -전쟁, -야구, -리버풀, -축구, -軍, -핵무기, -공포, -감독&lr=lang_ko&tbs=lr:lang_1ko,qdr:d&tbm=nws&source=lnt&sa=X&ved=0ahUKEwj0-oTnuenoAhUV62EKHa6pC9QQpwUIIQ&biw=1536&bih=754&dpr=1.25'
			...
			while (1):
				...
				news_entity = soup.select("div.gG0TJc")
				for j in range(len(news_entity)):			
					titles = news_entity[j].select("h3 > a")[0].text
					links = news_entity[j].select("h3 > a")[0].get("href")
					news = news_entity[j].select("div > span")[0].text
					news = news.strip()

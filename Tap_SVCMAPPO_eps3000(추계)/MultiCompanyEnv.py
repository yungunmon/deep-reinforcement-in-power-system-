...
class env():
    ...
    def reset(self):
        ...
        line32_33 = pp.create_line_from_parameters(net_ieee33, buses[31], buses[32], length_km=1, r_ohm_per_km = 0.3410, x_ohm_per_km = 0.5302, c_nf_per_km = 0, max_i_ka = 9, name='Line 32-33')
            ...
            # Ext-Grid
        pp.create_ext_grid(net_ieee33, buses[0], vm_pu=1.03, va_degree=0.0)
        ...
        pp.create_shunt(net_ieee33, buses[6], 0)
...
#        return PV, WT, LOAD, Pline, Pbus, radial
        ...
        for i in range (1,len(net_ieee33.res_bus)):
            x = -math.sqrt((1.0-net_ieee33.res_bus.vm_pu[i])**2)
            Vviol += x
        ...
        for i in range (6,18):
            x = -math.sqrt((1.0-net_ieee33.res_bus.vm_pu[i])**2)
            Vviol2 += x
        sta1 = np.array([net_ieee33.res_bus.vm_pu[5], net_ieee33.res_bus.vm_pu[22], net_ieee33.res_bus.vm_pu[23], net_ieee33.res_bus.vm_pu[24], net_ieee33.res_bus.vm_pu[25], net_ieee33.res_bus.vm_pu[26], net_ieee33.res_bus.vm_pu[27], net_ieee33.res_bus.vm_pu[28], net_ieee33.res_bus.vm_pu[29], net_ieee33.res_bus.vm_pu[30],net_ieee33.res_bus.vm_pu[31],net_ieee33.res_bus.vm_pu[32]])
        sta2 = np.array([net_ieee33.res_bus.vm_pu[6], net_ieee33.res_bus.vm_pu[7], net_ieee33.res_bus.vm_pu[8], net_ieee33.res_bus.vm_pu[9], net_ieee33.res_bus.vm_pu[10], net_ieee33.res_bus.vm_pu[11], net_ieee33.res_bus.vm_pu[12], net_ieee33.res_bus.vm_pu[13], net_ieee33.res_bus.vm_pu[14],net_ieee33.res_bus.vm_pu[15], net_ieee33.res_bus.vm_pu[16], net_ieee33.res_bus.vm_pu[17] ])
        state1 = np.hstack([pv_, load_, sta1 ]).flatten()
        state2 = np.hstack([pv_, load_, sta2 ]).flatten()
    ...
    def step(self, acts1,acts2):
        a1 = acts1
        a2 = acts2

        self.rwd = np.zeros(shape=(2,))
        terminal = False
        n_state, rwd, ifo , ifos, ifov= self._step(a1, a2)
        ...
        next_state = n_state
        total_reward = [self.rwd[0],self.rwd[1]]
    ...
    def _step(self,acts1,acts2):
        ...
        pp.create_sgen(net_ieee33, buses[2],  self.PV[self.time] , q_mvar=0,  name='PV 3', type='PV')
        pp.create_sgen(net_ieee33, buses[7],  self.PV[self.time] , q_mvar=0,  name='PV 8', type='PV')
        pp.create_sgen(net_ieee33, buses[13], self.PV[self.time] , q_mvar=0,  name='PV 14', type='PV')
        pp.create_sgen(net_ieee33, buses[24], self.PV[self.time] , q_mvar=0,  name='PV 25', type='PV')
        pp.create_sgen(net_ieee33, buses[29], self.PV[self.time] , q_mvar=0,  name='PV 29', type='PV')


        pp.create_sgen(net_ieee33, buses[30], self.PV[self.time] , q_mvar=0,  name='PV 30', type='PV')'''
        pp.create_shunt(net_ieee33, buses[6], acts2)
        pp.runpp(net_ieee33)
        ...
        total_reward =  Vviol 
        total_reward2 = Vviol2


        self.rwd = np.zeros(shape=(2,))        
        self.rwd[0] = total_reward + Pbus1
        self.rwd[1] = total_reward2 + Pbus2
